...
function validator(params) {
    return (req, res, next) => {
        ...
        if (!result.success) {
            const errorFormated = result.error.issues.map((item) => `${item.path.join('.')}: ${item.message}`);
            throw new app_error_1.AppError(errorFormated, http_status_codes_1.StatusCodes.UNPROCESSABLE_ENTITY);
        }
